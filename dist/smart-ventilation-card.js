...
const DEFAULTS = {type: 'custom:smart-ventilation-card', title: 'Airflow Card', show_title: true, animation: true,
  ...
  show_details: true, show_target_temperature: true, show_diagnostics: true, background_opacity: 1, calculate_efficiency: true, bypass_threshold: 1, bypass_active_state: 'on',
...
export function normalizeConfig(config) {
  ...
  for (const key of ['show_target_temperature', 'compact', 'show_status_text', 'animation', 'show_title', 'show_details', 'show_diagnostics', 'calculate_efficiency']) if (typeof c[key] !== 'boolean') throw new Error(`${key} must be true or false.`);
...
export class SmartVentilationCard extends HTMLElement {
  ...
  render() {
    ...
    const heatIndicator = offline ? `<g data-mode="offline"></g>`
      : bypass === 'active' ? `<g data-mode="bypass"><g class="heat-off" role="img" aria-label="Heat transfer off"><path class="heat-off-arrow" d="M176 111V105H170L180 92L190 105H184V106Z M176 123L184 118V134H176Z"/><path class="heat-off-slash" d="M166 127L194 108"/></g></g>`
    ...
    const footer = (key,label,icon) => `<button class="metric" data-entity="${escape(c.entities[key])}"><ha-icon icon="mdi:${icon}"></ha-icon><div><span>${escape(name(key,label))}</span><strong>${escape(this.value(e(key),key==='room_temperature'||key==='setpoint'))}</strong></div></button>`;
    const footerFields = [['room_temperature','Room','home-outline'],['humidity','Humidity','water-outline'],['level','Level','signal'],...(c.compact && c.show_target_temperature ? [['setpoint','Target','thermometer']] : [])].filter(([key])=>c.entities[key]);
    const diagnostics = ['setpoint','hygrostat','supply_rpm','extract_rpm','gateway','controller'].filter(k=>c.entities[k] && !(k==='setpoint' && c.show_details && footerFields.some(([key])=>key==='setpoint')));
    this.shadowRoot.innerHTML = `<style>${style}</style><ha-card style="--airflow-background-opacity:${c.background_opacity}" class="${c.animation ? '' : 'disabled'} ${c.show_title ? '' : 'headerless'} ${c.compact ? 'compact' : ''}">
      ...
      ${c.compact ? `<g transform="translate(50.4 -2) scale(.72)">${heatIndicator}</g>` : heatIndicator}
      ...
      ${c.show_details ? `<div class="metrics ${footerFields.length===4?'four-values':''}">${footerFields.map(([k,label,icon])=>footer(k,label,icon)).join('')}</div>` : ''}
...
export class SmartVentilationCardEditor extends HTMLElement {
  ...
  render() {
    ...
    this._form.schema = [
      ...
      {name:'show_target_temperature',label:'Show target temperature in compact footer',selector:{boolean:{}}},
